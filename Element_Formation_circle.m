function [k_e,f_e,n_loc] = Element_Formation_circle(e,nq,Xi_q,C,P,w_be,w_e,w_q,Neumann,Robin,k,f,h,beta,U_R,p,Xi_1,Xi_2,IEN)
k_e = zeros(nq^2);
f_e = zeros(nq^2,1);
n_loc = nq^2;
...
for q1 = 1:nq
    for q2 = 1:nq
        [Ra,dRa_dx,dRa_dy,~,J] = Shape_function_circle(e,Xi_q(q1),Xi_q(q2),n_loc,p,Xi_1,Xi_2,w_be,w_e(:,:,e),C,P,IEN);
        j = det(J);
...
for side = 1:4  % since we are only in 2D
    if Neumann(side,e) == 1 || Robin(side,e) == 1
        for q = 1:nq
            ...
            [Ra,~,~,~,J] = Shape_function_circle(e,xi_1,xi_2,n_loc,p,Xi_1,Xi_2,w_be,w_e,C,P,IEN);
