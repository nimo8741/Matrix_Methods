function [Ra,dRa_dx,dRa_dy,x,J] = Shape_function_circle(e,xi_1,xi_2,n_loc,p,Xi_1,Xi_2,w_be,w_ae,C,P,IEN)
%% Inputs
% e: element number
...
for a = 1:n_loc
    [i_2,i_1] = get_row_col(a,n_loc);  % I switched these outputs
...
for a = 1:n_loc
    ...
    for b = 1:n_loc
        Ra(a) = Ra(a) + w_ae_trans(IEN(a,1))*C{e}(a,b)*B(b)/w_b;
        dRa_dXi1(a) = dRa_dXi1(a) + w_ae_trans(IEN(a,1))*C{e}(a,b)*(dBa_dXi1(b)/w_b - dwb_dXi1*B(b)/(w_b^2));
        dRa_dXi2(a) = dRa_dXi2(a) + w_ae_trans(IEN(a,1))*C{e}(a,b)*(dBa_dXi2(b)/w_b - dwb_dXi2*B(b)/(w_b^2));
    end
end

% Physical Space Quantities
for a = 1:n_loc    % this does the modular arithmetic to get the correct row and column
    [col,row] = get_row_col(a,n_loc);    % should be row,col
